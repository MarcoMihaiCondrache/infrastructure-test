...
- [cloudflared](https://github.com/cloudflare/cloudflared) - Tunneling daemon that proxies traffic from the Cloudflare network to your origins. This daemon sits between Cloudflare network and your origin

_Additional applications include [emqx](https://github.com/toboshii/hajimari), [homeassistant](https://github.com/tarampampam/error-pages), [node-feature-discovery](https://github.com/Ealenn/Echo-Server), [node-red](https://github.com/rancher/system-upgrade-controller), [reloader](https://github.com/stakater/Reloader), and [zigbee2mqtt](https://github.com/weaveworks/kured)_

For provisioning the following tools will be used:

- [DietPi](https://dietpi.com/) - DietPi is an extremely lightweight Debian OS, highly optimised for minimal CPU and RAM resource usage.
- [Ansible](https://www.ansible.com) - Provision DietPi and install k3s
...
- One or more nodes with a fresh install of [DietPi](https://dietpi.com/).
- A [Cloudflare](https://www.cloudflare.com/) account with a domain, this will be managed by Terraform and the Advanced SSL Manager (optional). You can [register new domains](https://www.cloudflare.com/products/registrar/) directly thru Cloudflare.
- Some experience in debugging problems and a positive attitude ;)

📍 It is recommended to have at least 2 nodes for a decent cluster.
...
├─📁 core       # cluster core applications
...
📍 Here we will be running a Ansible Playbook to prepare DietPi for running a Kubernetes cluster.
...
5. Run the DietPi Ansible prepare playbook
...
📍 The terraform script will create the base dns to redirect all the traffic to you cluster tunnel so what you need is just to create you ingresses with the requested domain. Cloudflare will do the rest.

All deployed apps have by default a loadbalancer ip pointing the services so you can debug and test the applications locally!

Now if nothing is working, that is expected. This is DNS after all!

### 🔐 SSL

If your are adding ingresses to your applications with more than a sub domain like master.cluster.example.com cloudflare will not cover these domains in the default ssl certificates. You need to buy the Advanced SSL Manager for 10$/Month in order to allow your subdomains to be valid.
If you are using domains like hassio.example.com you are good with the free option (For Cloudflare Tunnels there are no other choices). 
...
Big special thanks to [@onedrop](https://github.com/onedr0p) for creating this beatiful and reusable template.